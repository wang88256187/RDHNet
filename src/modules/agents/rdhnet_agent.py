...
class RDHAgent(nn.Module):
    ...
    def __init__(self, input_shape, args):
        ...
        self.max_nums_refer_agent = args.max_nums_refer_agent
    ...
    def forward(self, inputs, hidden_state, actions):
        bs, own_feats, entity_pos, entity_relative_vel, \
            entity_dist, entity_vel_norm, angle_info, actions_norm, actions_angle_info = self._build_input(inputs,
                                                                                                           actions)
        # own_feats [bs, n_agents, own_feats_dim]
        # entity_pos [bs, n_agents, n_entities -1, 2]
        # entity_relative_vel [bs, n_agents, n_entities -1, n_entities -1, 2]
        # entity_dist [bs, n_agents, n_entities -1, 2]
        # angle_info [bs, n_agents, n_entities -1, n_entities -1, 2]
        ...
        all_feat_inputs = torch.cat([feat_dist, feat_angle, feat_node_feats], dim=-1)
        mask = entity_dist <= self.args.agent_view_radius + 0
        mask1 = mask.unsqueeze(2).repeat(1,1,self.n_entities-1, 1, self.hyper_input_dim)
        mask2 = mask.unsqueeze(3).repeat(1, 1, 1,self.n_entities-1,  self.hyper_input_dim)
        mask_for_feats = mask1 * mask2
        mask_for_action_angle = mask.repeat(1,1,1,2)
        # 选择参考实体的id
        # 随机选取
        if self.args.use_pomdp is False:
            referred_entity_id = torch.stack([torch.randperm(self.n_entities-1)[:self.max_nums_refer_agent] for _ in range(bs * self.n_agents)])
            index1 = (referred_entity_id.reshape(bs, self.n_agents, self.max_nums_refer_agent, 1, 1).
                                  repeat(1, 1, 1, self.n_entities - 1, all_feat_inputs.shape[-1])).cuda()

            index2 = (referred_entity_id.reshape(bs, self.n_agents, self.max_nums_refer_agent, 1).
                              repeat(1, 1, 1, 2)).cuda()
        else:
            # 按距离选取 for POMDP
            sorted_dist = entity_dist.sort(2)
            referred_entity_id = sorted_dist.indices[:, :, 0:self.max_nums_refer_agent]
            index1 = (referred_entity_id.reshape(bs, self.n_agents, self.max_nums_refer_agent, 1, 1).
                      repeat(1, 1, 1, self.n_entities - 1, all_feat_inputs.shape[-1])).cuda()
            ...
            # mask 视野外信息
            all_feat_inputs[mask_for_feats == 0] = 0
            actions_angle_info[mask_for_action_angle == 0] = 0

        feat_inputs = all_feat_inputs.gather(dim=2, index=index1)
        referred_actions_angle_info = actions_angle_info.gather(dim=2, index=index2)
        ...
        ally_out = ally_out.view(-1, self.n_agents, self.max_nums_refer_agent, self.n_entities - 1, self.head,
                                 self.hidden_dim)
        ...
        if self.n_enemies > 0:
            ...
            enemy_out = enemy_out.view(-1, self.n_agents, self.max_nums_refer_agent, self.n_entities - 1, self.head,
                                       self.hidden_dim)
        ...
        landmark_out = landmark_out.view(-1, self.n_agents, self.max_nums_refer_agent, self.n_entities - 1, self.head,
                                         self.hidden_dim)
        ...
        actions_norm = actions_norm.repeat(1, 1, self.max_nums_refer_agent).unsqueeze(-1)
        final_inputs = torch.cat([m_jki_, actions_norm, referred_actions_angle_info], dim=-1)
