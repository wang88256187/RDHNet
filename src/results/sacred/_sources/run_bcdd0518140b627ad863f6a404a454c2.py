...
def run(_run, _config, _log, pymongo_client=None):
    ...
    if args.use_wandb:
        wandb_logs_dir = os.path.join(dirname(dirname(abspath(__file__))), args.local_results_path, "wandb_logs")
        # wandb_exp_dir = os.path.join(wandb_logs_dir, "{}").format(unique_token)
...
def run_sequential(args, logger):
    ...
    if 'particle' not in args.env and "cts_matrix_game" not in args.env and "mujoco_multi" not in args.env and "mpe" not in args.env:
